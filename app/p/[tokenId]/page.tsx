...
async function loadEvents(tokenId: string, serial: number): Promise<EventRow[]> {
    ...
    <ol className="border-t border-b border-rule-strong">
      {events.map((e, i) => (
        ...
        >
          ...
          <div>
            <div className="font-display text-[22px] leading-none text-ink" style={{ fontWeight: 500 }}>
              {EVENT_LABEL(e.type)}
              {e.pending && (
    ...
    <article className="rise">
      <div className="mb-8 flex items-center justify-between text-[10px] tracking-[0.22em] uppercase text-ink-faint">
        ...
        <span className="font-mono numeral">
          N° {tokenId}/{String(serial).padStart(3, "0")}
        </span>
      </div>
      ...
      <div className="grid md:grid-cols-[1.1fr_1fr] gap-x-12 gap-y-12 mb-16">
        <section>
          ...
          <dl className="space-y-0">
            ...
            <DataRow label="GTIN" mono value={passport.gtin} />
            <DataRow
              label="Manufactured"
      ...
      <section className="pt-8 border-t-2 border-rule-strong">
        <div className="grid md:grid-cols-3 gap-6 text-[11px] font-mono text-ink-faint numeral">
          ...
          <div>
            ...
            <Link href="/repair" className="mt-2 inline-block underline text-accent decoration-rule-strong underline-offset-4 hover:decoration-accent">
              Log a repair ↗
            </Link>
          </div>
        </div>
